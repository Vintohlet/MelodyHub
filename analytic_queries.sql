...
SELECT 
    ...
    ROUND(SUM(il.unit_price * il.quantity), 2) AS revenue,
    EXTRACT(DAYS FROM (MAX(i.invoice_date) - MIN(i.invoice_date))) AS loyalty_days
FROM Customer c
JOIN Invoice i ON c.customer_id = i.customer_id
JOIN Invoice_Line il ON i.invoice_id = il.invoice_id
GROUP BY c.customer_id, c.first_name, c.last_name
HAVING EXTRACT(DAYS FROM (MAX(i.invoice_date) - MIN(i.invoice_date))) >= 1461 --Если с нами больше 4 лет 
ORDER BY revenue DESC;
